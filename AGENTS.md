...
For font effects, keep each `IFontEffect` implementation in its own `src/effects/` file and its editor in `src/components/effects/`. Every visual effect must expose `opacity`, apply it with `globalAlpha` inside `draw()`, serialize/deserialize it, and register both model and editor through the effect registries. Content effects draw into `getCurrentTargetContext()`: either the main canvas or the innermost active shadow group. `StartShadowEffect` pushes a group, and `End Shadow` pops it, draws its shadow into the parent target, then draws group content above that shadow. Do not add `instanceof` editor branching to `FontProperties.tsx` or canvas rendering.
