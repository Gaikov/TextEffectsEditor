...
Font effect models and canvas rendering live in the public npm package `grom-font-effects`. Do not add concrete effect classes back into this editor repo. The editor keeps only a thin `src/effects/index.ts` adapter for Blueprint icon metadata and MobX observability. New effects require model/serializer/deserializer/registry changes in the `Gaikov/GROmTextEffects` module repo, a published npm release, plus an editor component in `src/components/effects/` and an editor registry entry. Every effect must expose `visible` and every visual effect must expose `opacity`, apply it with `globalAlpha` inside `draw()`, and serialize/deserialize both fields. Use the composite pattern: `GroupEffect` is an `IFontEffect`, renders child effects into an offscreen buffer, and draws that buffer into its parent. Do not add `instanceof` editor branching to `FontProperties.tsx` or canvas rendering.
